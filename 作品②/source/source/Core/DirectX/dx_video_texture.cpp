...
struct VideoTexture::InternalData {
    ...
    Render_VideoTexture* target_texture = nullptr;
    uint32_t  width = 0;
    uint32_t  height = 0;
    float     fps = 0.0f;
...
public:
    ...
    void update(float elapsed) {
        ...
        if (pVideoSample) {
            ...
            if (!target_texture) {
                target_texture = new Render_VideoTexture();
...
bool VideoTexture::create(const char* filename, bool loop) {
    assert(!internal_data);
    internal_data = new InternalData();
    internal_data->setLoop(loop);
...
Render_VideoTexture* VideoTexture::getTexture() {
    assert(internal_data);
...
bool Render_VideoTexture::create(uint32_t new_xres, uint32_t new_yres, DXGI_FORMAT new_format, bool is_dynamic) {
    xres = new_xres;
    yres = new_yres;
    format = new_format;
    ...
    HRESULT hr = DXGetDevice()->CreateTexture2D(&desc, nullptr, (ID3D11Texture2D**)&texture);
    if (FAILED(hr))
        return false;
    ...
    hr = DXGetDevice()->CreateShaderResourceView(texture, &srv_desc, &shader_resource_view);
    if (FAILED(hr))
        return false;
...
void Render_VideoTexture::activate(int slot) const {
    DXGetDeviceContext()->PSSetShaderResources(slot, 1, &shader_resource_view);
}

void Render_VideoTexture::destroy() {
    SAFE_RELEASE(texture);
    SAFE_RELEASE(shader_resource_view);
}

bool Render_VideoTexture::updateFromIYUV(const uint8_t* data, size_t data_size)
{
    assert(data);
    D3D11_MAPPED_SUBRESOURCE ms;
    HRESULT hr = DXGetDeviceContext()->Map(texture, 0, D3D11_MAP_WRITE_DISCARD, 0, &ms);
    if (FAILED(hr))
        return false;
    ...
    DXGetDeviceContext()->Unmap(texture, 0);
    return true;
}