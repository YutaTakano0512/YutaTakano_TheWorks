...
float3 GetRandomPosition(uint instanceID)
{
    ...
    position.y = (1.0f - pow(baseY, 2.0)) * stageSize.y - stageSize.y * 0.75; // 二次関数を適用
...
float3 GetAnimatedPosition(uint instanceID, uint instanceIndex, float time)
{
    ...
    float3 movement;
    movement.x = sin(time * 0.01 * speed + instanceID * 0.2) * 5.2f;
    movement.y = sin(time * 0.08 * speed * 1.5) * 0.1f - 0.006f * ((time * speed * 1.2 + instanceID * 17.0f) % 1000.0f);
    movement.z = cos(time * 0.01 * speed + instanceID * 0.1) * 5.2f * 1000.0f;
    return movement;
}
