...
float3 GetRandomPosition(uint instanceID)
{
    ...
    position.z = frac(sin(seed * 1.3) * 43758.5453) * stageSize.z - stageSize.z * 0.35;

    // Y座標: Yが大きいほど分布密度を高くする
    float baseY = frac(sin(seed * 1.1) * 43578.5453);
    //position.y = pow(baseY, 4.0) * stageSize.y - stageSize.y * 0.3; // 二次関数を適用
    position.y = (1.0f - pow(baseY, 2.0)) * stageSize.y + stageSize.y * -0.12; // 二次関数を適用
...
float3 GetAnimatedPosition(uint instanceID, uint instanceIndex, float time)
{
    ...
    float3 movement;
    movement.x = sin(time * 0.008 * speed + instanceID * 0.2) * 0.3f;
    movement.y = sin(time * 0.001 * speed * 1.5) * 0.1f;
...
)
{
    ...
    / 1.0, 0.0), 1.0f)
    * max(pow(min(max(length(setp.x) / (96.0f * 0.105         ), 0.0), 1.0f), 1.0f), 0.1f)
    * max(pow(min(max(length(setp.z) / (96.0f * 0.400 * 1000.0), 0.0), 1.0f), 1.0f), 0.1f)
    ;
