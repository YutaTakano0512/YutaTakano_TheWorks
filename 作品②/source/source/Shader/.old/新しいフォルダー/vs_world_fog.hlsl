...
float3 GetRandomPosition(uint instanceID)
{
    ...
    float seed = instanceID * 132.456; // 固定のシード値
    float3 position;

    // X, Z座標は従来通り
    position.x = frac(sin(seed * 1.2) * 43758.5453) * stageSize.x - stageSize.x / 2;
    position.z = frac(sin(seed * 1.3) * 43758.5453) * stageSize.z - stageSize.z * 0.36;

    // Y座標: Yが大きいほど分布密度を高くする
    float baseY = frac(sin(seed * 1.1) * 43578.5453);
    //position.y = pow(baseY, 4.0) * stageSize.y - stageSize.y * 0.3; // 二次関数を適用
    position.y = (1.0f - pow(baseY, 2.0)) * stageSize.y + stageSize.y * -0.2; // 二次関数を適用
...
float3 GetAnimatedPosition(uint instanceID, uint instanceIndex, float time)
{
    time += -600.0 * instanceIndex;
    
    float speed = 0.2 + 0.3 * frac(sin(instanceID * 132.566) * 43758.5453); // 移動速度
    float3 movement;
    movement.x = sin(time * 0.010 * speed + instanceID * 0.2) * 0.09f;
    movement.y = sin(time * 0.001 * speed * 1.5) * 0.1f - 0.0001f * ((time * speed * 1.2 + instanceID * 17.0f) % 1000.0f);
    movement.z = cos(time * 0.016 * speed + instanceID * 0.1) * 0.01f * 1000.0f;
    return movement;
}
...
)
{
    ...
    / 1.0, 0.0), 1.0f)
    * max(pow(min(max(length(setp.x) / (96.0f * 0.015         ), 0.0), 1.0f), 1.0f), 0.4f)
    * max(pow(min(max(length(setp.z) / (96.0f * 0.100 * 1000.0), 0.0), 1.0f), 1.0f), 0.4f)
    ;
