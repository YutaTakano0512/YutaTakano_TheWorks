...
void main(in PS_IN In, out float4 outDiffuse : SV_Target)
{
    ...
    //float dis = 1; //length(CameraPos.xyz - In.Position.xyz);
    
    float2 uv = In.TexCoord * 3.0;
    float noise = FBM(uv) * 1.0;
    float baseAlpha = noise * TexColor.a * In.Diffuse.a 
    * SoftEdge(In.TexCoord, In.Position, index) * (sin(In.instanceID * 0.17) * 0.2 + 1.0);
    ...
    outDiffuse = baseAlpha <= EPSILON ? 0: float4(1, 1, 1, alpha) * float4(In.Diffuse.rgb * TexColor.rgb, 1.0f); // チェック柄の透明度を適用
