...
void UpdateCamera(void)
{
 ...
 /*   {
    ...
    for (int i = 0; i < CAMERA_NUM; i++)
    {
        ...
        //if (camera.mode == CAMERA_MODE_BOSS_CHASE)
        //{
        ...
        //            //GAME_OBJECT* bossObj = GetGameObject(GetBossbarObjIndex_Game());

        //            //if(bossObj != nullptr)
        //            {
        //                float m = 1.0f / 36.0f;

        //                float ax = (float)(plOb->pos.x - camera.trans.target.x);
        //                float ay = (float)((plOb->pos.y + 9000.0f) - camera.trans.target.y);
        //                float az = (float)(plOb->pos.z - camera.trans.target.z);

        //                camera.trans.target.x += (float)(ax * m);
        //                camera.trans.target.y += (float)(ay * m);
        //                camera.trans.target.z += (float)(az * m);
        ...
        //                float ax = (float)(
        //                    (plOb->pos.x + plOb->vel.x  + CAMERA_PLAYER_CHASE_POS_X)
        //                    - camera.trans.pos.x);
        //                float ay = (float)(
        //                    (plOb->pos.y + plOb->vel.y + CAMERA_PLAYER_CHASE_POS_Y - 100.0f)
        //                    - camera.trans.pos.y);
        //                float az = (float)(
        //                    (plOb->pos.z + plOb->vel.z + CAMERA_PLAYER_CHASE_POS_Z + 16000)
        //                    - camera.trans.pos.z);

        //                camera.trans.pos.x += (float)(ax * m);
        //                camera.trans.pos.y += (float)(ay * m);
        //                camera.trans.pos.z += (float)(az * m);
        ...
        //                //camera.trans.zoom += (float)(CAMERA_PLAYER_CHASE_ZOOM * 0.65f - camera.trans.zoom) * m;
        //                camera.trans.zoom += (float)(0.42f * 0.65f - camera.trans.zoom) * m;
        //            }
        //        }
        //    }
        //}
        if (camera.mode == CAMERA_MODE_PLAYER_CHASE || camera.mode == CAMERA_MODE_BOSS_CHASE)
        {
            bool isBoss = camera.mode == CAMERA_MODE_BOSS_CHASE;
            ...
            if (pl != nullptr)
            {
                ...
                if (plOb != nullptr)
                {
                    ...
                    //float mpo = 1.0f / 42.0f;
                    //float mz = 1.0f / 65.0f;
                    float mpo = 1.0f / 65.0f;
                    float mpota = 1.0f / 18.0f;
                    float mz = 1.0f / 32.0f;
                    ...
                    { 
                        ...
                        if (goe->IsDrawing() || kakomain.dat.IsSuccess)
                        //if (goe->IsDrawing())
                        {
                            ...
                            cpostmp.x = po.x + plbeforeVec.x * BLOCK_WIDTH * (8.0f + spd * 0.6f) * cpc.vel + pfvec.x * dis / 8;
                            cpostmp.y = po.y + plbeforeVec.y * BLOCK_WIDTH * (8.0f + spd * 0.6f) * cpc.vel + pfvec.y * dis / 8;
                            cpostmp.z = 0.0f + plbeforeVec.z * BLOCK_WIDTH * (8.0f + spd * 0.6f) * cpc.vel;
                        ...
                        {
                            float bossvecmod = isBoss ? 1.2f : 1.0f;
                            cpostmp.x = plOb->pos.x + plbeforeVec.x * BLOCK_WIDTH * (4.0f + spd * 1.0f) * bossvecmod * cpc.vel;
                            cpostmp.y = plOb->pos.y + plbeforeVec.y * BLOCK_WIDTH * (4.0f + spd * 1.0f) * bossvecmod * cpc.vel;
                            cpostmp.z = plOb->pos.z + plbeforeVec.z * BLOCK_WIDTH * (4.0f + spd * 1.0f) * bossvecmod * cpc.vel;
                        ...
                        cpostmp.x += CAMERA_PLAYER_CHASE_POS_X;
                        cpostmp.y += CAMERA_PLAYER_CHASE_POS_Y;
                        cpostmp.z += CAMERA_PLAYER_CHASE_POS_Z;

                        if (stageSize.x > 0 && stageSize.y > 0)
                        {
                            float stageW = BLOCK_WIDTH * stageSize.x - (SCREEN_WIDTH * 1 * (1.0f / czoom) / 2);
                            float stageH = BLOCK_HIGHT * stageSize.y - (SCREEN_HEIGHT * 1 * (1.0f / czoom) / 2);
                    ...
                    }
                    {
                        ...
                        //camera.trans.zoom = EasingOutQuart( camera.trans.zoom +  addzoom* (addzoom > 0 ? 0.16f : 1.0f));
                    }

                    {
                        //急激に角度が変わった場合は加速度を緩める
                        float degc = CalculateRadToDeg(atan2(plbeforeVec.y, plbeforeVec.x));
                        //while (degc < 0.0f) degc += 360.0f;
                        //while (degc > 360.0f) degc += -360.0f;

                        float degb = CalculateRadToDeg(atan2(cpc.playerBeforeVec.y, cpc.playerBeforeVec.x));
                        //while (degb < 0.0f) degb += 360.0f;
                        //while (degb > 360.0f) degb += -360.0f;

                        float degsub = abs(degc - degb);
                        if (degsub >= 50)
                        {
                            cpc.velSubRequ = cpc.vel * (1.0f - 0.1f);
                            cpc.velSubRequSpd = 1.0f;
                        }
                        else if (degsub > 0)
                        {
                            cpc.velSubRequ = cpc.vel
                                * (1.0f - std::min(std::max(
                                    1.0f - std::min(std::max(degsub / 180.0f, 0.0f), 1.0f),
                                    0.1f), 0.9f)
                                  );
                            cpc.velSubRequSpd = 9.0f;
                        }
                    }

                    if (cpc.velSubRequ >= 0.1)
                    {
                        float sub = cpc.velSubRequ / -cpc.velSubRequSpd;
                        cpc.vel += sub;
                        cpc.velSubRequ += sub;
                    }
                    //else
                    {
                        ...
                        //if (cpc.playerBeforeSpd <= 1.0f && spd > 1.0f)
                        //{
                        //    cpc.vel = 0.1f;
                        //}

                        if (cpc.vel > 0.0f 
                            && (CalculateDistance(camera.trans.pos, cpostmp) <= 1.0f || spd < 8.0f)
                        )
                        {
                            cpc.vel += 1.0f / -200.0f;
                        }
                    ...
                    {
                        //float mspd = 20.0f;
                        //float mdis = CalculateDistance(cpc.cameraPosBuf, camera.trans.pos);
                        //Float3 ve = {};
                        //ve.x = (float)((cpc.cameraPosBuf.x) - camera.trans.pos.x);
                        //ve.y = (float)((cpc.cameraPosBuf.y) - camera.trans.pos.y);
                        //ve.z = (float)((cpc.cameraPosBuf.z) - camera.trans.pos.z);
                        //ve = NormalizeVector(ve);
                        ...
                        //camera.trans.pos.x += ve.x * mspd;
                        //camera.trans.pos.y += ve.y * mspd;
                        //camera.trans.pos.z += ve.z * mspd;

                        camera.trans.pos = cpc.cameraPosBuf;
                    }
                    {
                        //float mspd = 20.0f;
                        //float mdis = CalculateDistance(cpc.cameraTargetBuf, camera.trans.target);
                        //Float3 ve = {};
                        //ve.x = (float)((cpc.cameraTargetBuf.x) - camera.trans.target.x);
                        //ve.y = (float)((cpc.cameraTargetBuf.y) - camera.trans.target.y);
                        //ve.z = (float)((cpc.cameraTargetBuf.z) - camera.trans.target.z);
                        //ve = NormalizeVector(ve);
                        ...
                        //camera.trans.target.x += ve.x * mspd;
                        //camera.trans.target.y += ve.y * mspd;
                        //camera.trans.target.z += ve.z * mspd;
        ...
        if (deSave)
        {
            ...
            {
                vec.x = cinfo.CameraScalingRaw.x / 20;
                vec.y = cinfo.CameraScalingRaw.y / 20;
                vec.z = cinfo.CameraScalingRaw.z / 20;
...
void CameraShakeRequestAll(int frame, float power, float powerY)
{
    for (int i = 0; i < CAMERA_NUM; i++)
    {
        CameraShakeRequest(i, frame, power, powerY);
    }
}
void CameraShakeRequest(int index, int frame, float power, float powerY)
{
    CAMERA& camera = g_Camera[index];
