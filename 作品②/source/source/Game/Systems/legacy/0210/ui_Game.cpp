...
//インク
#define INKBAR_SCALE						(0.6f)
#define INKBAR_OFFSET_X						(SCREEN_WIDTH * -0.325f)
		...
		if (UIDat.hpbar[i].enable)
		{
			...
			if (player->obj != nullptr)
			{
				...
				//プレイヤーのインク
				{
                    ...
                    Int2 ink = ((GOE_Character_Player*)player->obj->events.source)->GetKakoponInkGage();
...
void DrawUI_Game()
{
    ...
    //if (UIDat.score.enable)
    {
        ...
        float addsaddy = (SCORE_SIZE_Y * -0.16f
            + (float)sin((float)GetMainGameFrame() * 0.7f) * SCORE_SIZE_Y * -0.08f)
            * EasingOutBack(addsm);

        Float4 numcolor = MkF4(1.1f, 1.1f, 1.1f, 1.0f);
        if (addsAlpha > 0)
        {
            numcolor.x += 0.9f * addsAlpha;
            numcolor.y += 0.9f * addsAlpha;
            numcolor.z += 0.9f * addsAlpha;
        }

        DrawSpriteQuadColorRotateTexUv(
            //表示座標 (四角形の真ん中)
            SCORE_OFFSET_X + SCORE_SIZE_X * -1.0f,
            SCORE_OFFSET_Y + SCORE_SIZE_Y * 0.6f,
        ...
        //DrawSpriteQuadColorRotateTexUv(
        //    //表示座標 (四角形の真ん中)
        //    SCORE_OFFSET_X + SCORE_SIZE_X * -3.5f,
        //    SCORE_OFFSET_Y + SCORE_SIZE_Y * 0.0f,
        ...
        //    //色
        //    MkF4(1.5f, 1.5f, 1.5f, 1.0f),

        //    g_tex_scoretext2.textureId,  //画像(テクスチャ)の番号
        //    MkF2(0.0f, 0.0f),
        //    MkF2(1.0f, 1.0f)
        //);
        SpriteNumberDraw(UIDat.score.score,
            MkF2(SCORE_OFFSET_X + SCORE_SIZE_X * 0.62f + 8.0f,
                SCORE_OFFSET_Y + SCORE_SIZE_Y * 0.1f + 8.0f + addsaddy),
            MkF2(SCORE_SIZE_X, SCORE_SIZE_Y),
            5, MkF4(-1.0f, -1.0f, -1.0f, 0.75f), -20.0f);
        SpriteNumberDraw(UIDat.score.score,
            MkF2(SCORE_OFFSET_X + SCORE_SIZE_X * 0.62f,
                SCORE_OFFSET_Y + SCORE_SIZE_Y * 0.1f + addsaddy),
            MkF2(SCORE_SIZE_X, SCORE_SIZE_Y),
            5, numcolor, -20.0f);
            ...
            for (int i = 0; i < SCORE_ADD_DAT_MAX; i++)
            {
                if (UIDat.score.scoreAddDat[i].enable)
                {
                    ...
                    for (int j = 0; j < 2; j++)
                    {
                        ...
                        SpriteNumberDraw(value,
                            MkF2(adp.x + SCORE_OFFSET_X + SCORE_SIZE_X * -2.5f,
                                adp.y + SCORE_OFFSET_Y + SCORE_SIZE_Y * 0.1f + SCORE_SIZE_Y * UIDat.score.scoreAddDat[i].index + (SCORE_SIZE_Y * 0.6f - SCORE_SIZE_Y * 0.6f * EasingOutElastic(1.0f - m))),
                            MkF2(SCORE_SIZE_X * 0.6f, SCORE_SIZE_Y * 0.6f),
                            -1, ct, -12.0f, true);
                    }
...
void update_player_gui(unsigned int gframe, int i, int cIndex, Float4 colorAlphaSetTmp, float movieFlagAlpha,
    bool isSub, int subindex
)
{
    Float2 subPos = {};
    ...
    {
        subPosBar.x += -150.0f;
        subPosBar.y += -80.0f * subindex - 180.0f;
        subPosBar_Ink.y += 20.0f;

        subPosIcon.x += -20.0f;
        subPosIcon.y += -80.0f * subindex - 130.0f;
    ...
    //プレイヤーのHP
    {
        ...
        for (int j = 0; j < 4; j++)
        {
            ...
            DrawSpriteInCameraSingle(
                cIndex,
                //表示座標 (四角形の真ん中)
                hb->bar.pos.x + (pos.x + ((float)shakextmp) - size.x / 2 * (1.0f - barmod)) * subScale.x /*+ (SCREEN_WIDTH / (SCREEN_WIDTH / HPBAR_SIZE_X)) * i*/ + subPosBar.x,
                hb->bar.pos.y + (pos.y + ((float)shakeytmp)) * subScale.y + subPosBar.y,

                //サイズ width hight
                size.x * barmod * subScale.x,
                size.y * subScale.y,
    ...
    //プレイヤーのインクゲージ
    {
        ...
        int indBefmax = 5;
        int indmax = indBefmax + (GOE_Character_Player::KAKOPON_INK_GAGE_MEMORY_NUM - 1);
        for (int j = 0; j < indmax; j++)
        {
            ...
            DrawSpriteInCameraSingle(
                cIndex,
                //表示座標 (四角形の真ん中)
                ib->bar.pos.x + (pos.x + ((float)shakextmp) - size.x / 2 * (1.0f - barmod)) * subScale.x 
                /*+ (SCREEN_WIDTH / (SCREEN_WIDTH / INKBAR_SIZE_X)) * i*/ + subPosBar.x + subPosBar_Ink.x,
                ib->bar.pos.y + (pos.y + ((float)shakeytmp))* subScale.y + subPosBar.y + subPosBar_Ink.y,
